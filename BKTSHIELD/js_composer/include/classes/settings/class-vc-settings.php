...
class Vc_Settings {
	...
	public function setTabs() {
		...
		if ( $this->showConfigurationTabs() ) {
			$this->tabs['vc-general'] = __( 'General Settings', 'blackfyre' );
			if ( ! vc_is_as_theme() || apply_filters( 'vc_settings_page_show_design_tabs', false ) ) {
				$this->tabs['vc-color'] = __( 'Design Options', 'blackfyre' );
				// $this->tabs['element_css'] = __('Element Class Names', 'blackfyre');
				$this->tabs['vc-custom_css'] = __( 'Custom CSS', 'blackfyre' );
			}
		}

		if ( ! vc_is_network_plugin() || ( vc_is_network_plugin() && is_network_admin() ) ) {
			if ( ! vc_is_updater_disabled() ) {
				$this->tabs['vc-updater'] = __( 'Product License', 'blackfyre' );
			}
		}
	...
	public function initAdmin() {
		...
		self::$color_settings = array(
			array( 'vc_color' => array( 'title' => __( 'Main accent color', 'blackfyre' ) ) ),
			array( 'vc_color_hover' => array( 'title' => __( 'Hover color', 'blackfyre' ) ) ),
			array( 'vc_color_call_to_action_bg' => array( 'title' => __( 'Call to action background color', 'blackfyre' ) ) ),
			//array('vc_color_call_to_action_border' => array('title' =>__('Call to action border color', 'blackfyre'))),
			array( 'vc_color_google_maps_bg' => array( 'title' => __( 'Google maps background color', 'blackfyre' ) ) ),
			array( 'vc_color_post_slider_caption_bg' => array( 'title' => __( 'Post slider caption background color', 'blackfyre' ) ) ),
			array( 'vc_color_progress_bar_bg' => array( 'title' => __( 'Progress bar background color', 'blackfyre' ) ) ),
			array( 'vc_color_separator_border' => array( 'title' => __( 'Separator border color', 'blackfyre' ) ) ),
			array( 'vc_color_tab_bg' => array( 'title' => __( 'Tabs navigation background color', 'blackfyre' ) ) ),
			array( 'vc_color_tab_bg_active' => array( 'title' => __( 'Active tab background color', 'blackfyre' ) ) )
		);
		self::$defaults = array(
			'vc_color' => '#f7f7f7',
		...
		// Use custom checkbox
		$this->addField( $tab, __( 'Use custom design options', 'blackfyre' ), 'use_custom', array(
			&$this,
		...
		// Margin
		$this->addField( $tab, __( 'Elements bottom margin', 'blackfyre' ), 'margin', array(
			&$this,
			'sanitize_margin_callback'
		), array( &$this, 'margin_callback' ) );

		// Gutter
		$this->addField( $tab, __( 'Grid gutter width', 'blackfyre' ), 'gutter', array(
			&$this,
			'sanitize_gutter_callback'
		), array( &$this, 'gutter_callback' ) );

		// Responsive max width
		$this->addField( $tab, __( 'Mobile screen width', 'blackfyre' ), 'responsive_max', array(
			&$this,
		...
		$tab = 'element_css';
		$this->addSection( $tab );
		$this->addField( $tab, __( 'Row CSS class name', 'blackfyre' ), 'row_css_class', array(
			&$this,
			'sanitize_row_css_class_callback'
		), array( &$this, 'row_css_class_callback' ) );
		$this->addField( $tab, __( 'Columns CSS class names', 'blackfyre' ), 'column_css_classes', array(
			&$this,
		...
		$tab = 'updater';
		$this->addSection( $tab );
		$this->addField( $tab, __( 'Envato Username', 'blackfyre' ), 'envato_username', array(
			&$this,
			'sanitize_envato_username'
		), array( &$this, 'envato_username_callback' ) );
		$this->addField( $tab, __( 'Secret API Key', 'blackfyre' ), 'envato_api_key', array(
			&$this,
			'sanitize_envato_api_key'
		), array( &$this, 'envato_api_key_callback' ) );
		$this->addField( $tab, __( 'Visual Composer License Key', 'blackfyre' ), 'js_composer_purchase_code', array(
			&$this,
	...
	public function restoreColor() {
		...
		// wp_redirect( ! empty( $_POST['_wp_http_referer'] ) ? preg_replace( '/tab\=/', 'tab_old=', $_POST['_wp_http_referer'] ) . '&tab=color' : '/' );
	}
	...
	function adminLoad() {
		...
		$this->locale = array(
			'are_you_sure_reset_css_classes' => __( 'Are you sure you want to reset to defaults?', 'blackfyre' ),
			'are_you_sure_reset_color' => __( 'Are you sure you want to reset to defaults?', 'blackfyre' ),
			'vc_updater_error' => __( 'Something went wrong! Please try again later.', 'blackfyre' ),
			'vc_updater_license_activation_success' => __( 'License successfully activated. Thank you!', 'blackfyre' ),
			'vc_updater_license_deactivation_success' => __( 'Your license key is deactivated.', 'blackfyre' ),
			'vc_updater_empty_data' => __( 'Envato username and license key are required.', 'blackfyre' ),
			'vc_updater_wrong_license_key' => __( 'Invalid liense key. Please check information in your envato profile.', 'blackfyre' ),
			'vc_updater_wrong_data' => __( 'Wrong data. Please check your information or ask support for help.', 'blackfyre' ),
			'vc_updater_already_activated' => __( 'License successfully activated. Thank you!', 'blackfyre' ),
			'vc_updater_already_activated_another_url' => sprintf( __( 'Your license key is already activated on another website ({site}), you should deactivate it first or <a href="%s" target="_blank">obtain new license key</a>.', 'blackfyre' ), esc_url( "http://bit.ly/vcomposer" ) ),
			'vc_updater_activate_license' => __( 'Activate License', 'blackfyre' ),
			'vc_updater_deactivate_license' => __( 'Deactivate License', 'blackfyre' ),
			'wrong_username_api_key' => __( 'Wrong username and/or api key!', 'blackfyre' ),
			'saving' => __( 'Saving...', 'blackfyre' ),
			'save' => __( 'Save Changes', 'blackfyre' ),
			'saved' => __( 'Design Options successfully saved.', 'blackfyre' ),
			'save_error' => __( 'Design Options could not be saved', 'blackfyre' ),
			'form_save_error' => __( 'Problem with AJAX request execution, check internet connection and try again.', 'blackfyre' ),
		);
	...
	public function groups_access_rules_callback() {
		...
		$show_types = array(
			'all' => __( 'Show Visual Composer & default editor', 'blackfyre' ),
			'only' => __( 'Show only Visual Composer', 'blackfyre' ),
			'no' => __( "Don't allow to use Visual Composer", 'blackfyre' )
		);
				...
				if ( ( isset( $params['capabilities']['edit_posts'] ) && $params['capabilities']['edit_posts'] === true )
					...
					<div id="wpb-settings-group-<?php echo $key ?>" class="accordion-body">
						<div class="visibility settings-block">
							<label
								for="wpb_composer_access_<?php echo $key ?>"><b><?php _e( 'Visual Composer access', 'blackfyre' ) ?></b></label>
						...
						<div class="shortcodes settings-block">
							<div class="title"><b><?php _e( 'Enabled shortcodes', 'blackfyre' ); ?></b></div>
							<?php $z = 1;
								...
								<?php if ( ! in_array( $el['base'], array(
									...
									<label>
										...
										<?php _e( $el["name"], "js_composer" ) ?><?php if ( isset( $el['deprecated'] ) && $el['deprecated'] !== false ) {
											echo ' <i>' . sprintf( __( '(deprecated since v%s)', 'blackfyre' ), $el['deprecated'] ) . '</i>';
										} ?>
									</label>
						...
						<div class="select-all">
							<a href="#"
							   class="wpb-settings-select-all-shortcodes"><?php echo __( 'Select All', 'blackfyre' ) ?></a>
							| <a href="#"
							     class="wpb-settings-select-none-shortcodes"><?php echo __( 'Select none', 'blackfyre' ) ?></a>
						</div>
					</div>
					</div>
				<?php
	...
	public function row_css_class_callback() {
		...
		// echo '<p class="description indicator-hint">'.__('To change class name for the row element, enter it here. By default vc_row is used.', 'blackfyre').'</p>';
	...
	public function margin_callback() {
		...
		echo '<p class="description indicator-hint css-control">' . __( 'Change default vertical spacing between content elements (Example: 20px).', 'blackfyre' ) . '</p>';
	}
	...
	public function gutter_callback() {
		...
		echo '<p class="description indicator-hint css-control">' . __( 'Change default horizontal spacing between columns, enter new value in pixels.', 'blackfyre' ) . '</p>';
	}
	...
	public function responsive_max_callback() {
		...
		$value = ( $value = get_option( self::$field_prefix . $field ) ) ? $value : $this->getDefault( $field );
		echo '<input type="text" name="' . self::$field_prefix . $field . '" value="' . $value . '" class="css-control"> px';
		echo '<p class="description indicator-hint css-control">' . __( 'By default content elements "stack" one on top other when screen size is smaller than 768px. Change the value to change "stacking" size.', 'blackfyre' ) . '</p>';
	}
	...
	public function envato_username_callback() {
		...
		echo '<p class="description indicator-hint">' . __( 'Enter your Envato username.', 'blackfyre' ) . '</p>';
	}
	...
	public function js_composer_purchase_code_callback() {
		...
		echo '<p class="description indicator-hint">' . sprintf( __( 'Please enter your CodeCanyon Visual Composer license key, you can find your key by following the instructions on <a href="%s" target="_blank">this page</a>. (Example of license key: bjg759fk-kvta-6584-94h6-75jg8vblatftq)', 'blackfyre' ), esc_url( "http://go.wpbakery.com/purchase-code" ) ) . '</p>';
	}
	...
	public function envato_api_key_callback() {
		...
		echo '<p class="description indicator-hint">' . sprintf( __( "Enter your API key, you can find your API key by following the instructions on <a href='%s' target='_blank'>this page</a>.", 'blackfyre' ), esc_url( "http://go.wpbakery.com/faq-api-key" ) ) . '</p>';
	}
	...
	public function setting_section_callback_function( $tab ) {
		if ( $tab["id"] == 'wpb_js_composer_settings_color' ): ?>
			<div class="tab_intro">
				<p>
					<?php _e( 'Here you can tweak default Visual Composer content elements visual appearance. By default Visual Composer is using neutral light-grey theme. Changing "Main accent color" will affect all content elements if no specific "content block" related color is set.', 'blackfyre' ) ?>
				</p>
			</div>
			...
			<div class="tab_intro">
				<?php if ( vc_is_as_theme() ): ?>
					<div class="updated inline">
						<p>
							<?php _e( 'Please activate your license in Product License tab!', 'blackfyre' ) ?>
						</p>
					</div>
				<?php endif; ?>
				<p>
					<?php //_e('Add your Envato credentials, to enable auto updater. With correct login credentials Visual Composer will be updated automatically (same as other plugins do).', 'blackfyre') ?>
					<?php echo sprintf( __( 'A valid license key qualifies you for support and enables automatic updates. <strong>A license key may only be used for one Visual Composer installation on one WordPress site at a time.</strong> If you previosly activated your license key on another site, then you should deactivate it first or <a href="%s" target="_blank">obtain new license key</a>.', 'blackfyre' ), esc_url( "http://bit.ly/vcomposer" ) ); ?>
				</p>
			</div>
		<?php endif;
	}
	...
	public function sanitize_margin_callback( $margin ) {
		...
		if ( ! preg_match( '/^\d+(px|%|em|pt){0,1}$/', $margin ) ) {
			add_settings_error( self::$field_prefix . 'margin', 1, __( 'Invalid Margin value.', 'blackfyre' ), 'error' );
		}
	...
	public function sanitize_gutter_callback( $gutter ) {
		...
		if ( ! $this->_isGutterValid( $gutter ) ) {
			add_settings_error( self::$field_prefix . 'gutter', 1, __( 'Invalid Gutter value.', 'blackfyre' ), 'error' );
		}
	...
	public function sanitize_responsive_max_callback( $responsive_max ) {
		if ( ! $this->_isNumberValid( $responsive_max ) ) {
			add_settings_error( self::$field_prefix . 'responsive_max', 1, __( 'Invalid "Responsive max" value.', 'blackfyre' ), 'error' );
		}
	...
	public function showNotification() {
		echo '<div class="error"><p>' . sprintf( __( 'Visual Composer: Your css class names settings are deprecated. <a href="%s">Click here to resolve</a>.', 'blackfyre' ), menu_page_url( $this->page, false ) . '&tab=element_css' ) . '</p></div>';
	}
	...
	public function output() {
		...
		<div class="wrap vc_settings" id="wpb-js-composer-settings">
			<h2><?php _e( 'Visual Composer Settings', 'blackfyre' ); ?></h2>
			<?php
			?>
...
EOF;
				...
				<?php
					...
					<form action="options.php" method="post" id="vc_settings-<?php echo $tab ?>"
						...
						?>
						<?php if ( $tab !== 'updater' ): ?>
							<?php submit_button( __( 'Save Changes', 'blackfyre' ), 'primary', 'submit', true, $submit_button_attributes ); ?>
						<?php endif; ?>
						<input type="hidden" name="vc_action" value="<?php echo 'vc_action-' . $tab; ?>"
						       id="vc_settings-<?php echo $tab ?>-action"/>
						<?php if ( $tab == 'color' ): ?>
							<a href="#" class="button vc_restore-button"
							   id="vc_settings-color-restore-default"><?php _e( 'Restore Default', 'blackfyre' ) ?></a>
						<?php endif; ?>
						<?php if ( $tab === 'updater' ): ?>
							...
							<a href="#" class="button button-primary vc_activate-license-button"
							   id="vc_settings-activate-license"><?php empty( $license_activation_key ) ? _e( 'Activate License', 'blackfyre' ) : _e( 'Deactivate License', 'blackfyre' ) ?></a>
		...
		if ( strlen( trim( $css_string ) ) > 0 ) {
			...
			if ( ! $wp_filesystem->put_contents( $filename, $css_string, FS_CHMOD_FILE ) ) {
				if ( is_wp_error( $wp_filesystem->errors ) && $wp_filesystem->errors->get_error_code() ) {
					add_settings_error( self::$field_prefix . 'main_color', $wp_filesystem->errors->get_error_code(), __( 'Something went wrong: js_composer_front_custom.css could not be created.', 'blackfyre' ) . ' ' . $wp_filesystem->errors->get_error_message(), 'error' );
				} elseif ( ! $wp_filesystem->connect() ) {
					add_settings_error( self::$field_prefix . 'main_color', $wp_filesystem->errors->get_error_code(), __( 'js_composer_front_custom.css could not be created. Connection error.', 'blackfyre' ), 'error' );
				} elseif ( ! $wp_filesystem->is_writable( $filename ) ) {
					add_settings_error( self::$field_prefix . 'main_color', $wp_filesystem->errors->get_error_code(), sprintf( __( 'js_composer_front_custom.css could not be created. Cannot write custom css to "%s".', 'blackfyre' ), $filename ), 'error' );
				} else {
					add_settings_error( self::$field_prefix . 'main_color', $wp_filesystem->errors->get_error_code(), __( 'js_composer_front_custom.css could not be created. Problem with access.', 'blackfyre' ), 'error' );
				}
				delete_option( self::$field_prefix . 'use_custom' );
	...
	public static function buildCustomCss() {
		...
		if ( ! $wp_filesystem->put_contents( $filename, $css_string, FS_CHMOD_FILE ) ) {
			if ( is_wp_error( $wp_filesystem->errors ) && $wp_filesystem->errors->get_error_code() ) {
				add_settings_error( self::$field_prefix . 'custom_css', $wp_filesystem->errors->get_error_code(), __( 'Something went wrong: custom.css could not be created.', 'blackfyre' ) . $wp_filesystem->errors->get_error_message(), 'error' );
			} elseif ( ! $wp_filesystem->connect() ) {
				add_settings_error( self::$field_prefix . 'custom_css', $wp_filesystem->errors->get_error_code(), __( 'custom.css could not be created. Connection error.', 'blackfyre' ), 'error' );
			} elseif ( ! $wp_filesystem->is_writable( $filename ) ) {
				add_settings_error( self::$field_prefix . 'custom_css', $wp_filesystem->errors->get_error_code(), __( 'custom.css could not be created. Cannot write custom css to "' . $filename . '".', 'blackfyre' ), 'error' );
			} else {
				add_settings_error( self::$field_prefix . 'custom_css', $wp_filesystem->errors->get_error_code(), __( 'custom.css could not be created. Problem with access.', 'blackfyre' ), 'error' );
			}
	...
	public static function checkCreateUploadDir( $wp_filesystem, $option, $filename ) {
		...
		if ( ! $wp_filesystem->is_dir( $js_composer_upload_dir ) ) {
			if ( ! $wp_filesystem->mkdir( $js_composer_upload_dir, 0777 ) ) {
				add_settings_error( self::$field_prefix . $option, $wp_filesystem->errors->get_error_code(), __( sprintf( '%s could not be created. Not available to create js_composer directory in uploads directory (' . $js_composer_upload_dir . ').', $filename ), 'blackfyre' ), 'error' );
	...
	protected static function getFileSystem( $url = '' ) {
		...
		if ( false === ( $creds = request_filesystem_credentials( $url, '', false, false, null ) ) ) {
			_e( 'This is required to enable file writing for js_composer', 'blackfyre' );
