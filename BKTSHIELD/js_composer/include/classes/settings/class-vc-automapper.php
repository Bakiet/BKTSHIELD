...
if ( ! class_exists( 'Vc_Automapper' ) ) {
	...
	class Vc_Automapper {
		...
		public function __construct() {
			$this->title = __( 'My Shortcodes', 'blackfyre' );
		}
		...
		public function build() {
			...
			wp_localize_script( 'wpb_js_composer_automapper', 'i18nLocaleVcAutomapper', array(
				'are_you_sure_delete' => __( 'Are you sure you want to delete this shortcode?', 'blackfyre' ),
				'are_you_sure_delete_param' => __( "Are you sure you want to delete the shortcode's param?", 'blackfyre' ),
				'my_shortcodes_category' => __( 'My shortcodes', 'blackfyre' ),
				'error_shortcode_name_is_required' => __( "Shortcode name is required.", 'blackfyre' ),
				'error_enter_valid_shortcode_tag' => __( "Please enter valid shortcode tag.", 'blackfyre' ),
				'error_enter_required_fields' => __( "Please enter all required fields for params.", 'blackfyre' ),
				'new_shortcode_mapped' => __( 'New shortcode mapped from string!', 'blackfyre' ),
				'shortcode_updated' => __( 'Shortcode updated!', 'blackfyre' ),
				'error_content_param_not_manually' => __( 'Content param can not be added manually, please use checkbox.', 'blackfyre' ),
				'error_param_already_exists' => __( 'Param %s already exists. Param names must be unique.', 'blackfyre' ),
				'error_wrong_param_name' => __( 'Please use only letters, numbers and underscore for param name', 'blackfyre' ),
				'error_enter_valid_shortcode' => __( 'Please enter valid shortcode to parse!', 'blackfyre' )
			) );
		}
		...
		public function renderHtml() {
			...
			?>
			<div class="tab_intro">
				<p><?php _e( 'Visual Composer Shortcode Mapper adds custom 3rd party vendors shortcodes to the list of Visual Composer content elements menu (Note: to map shortcode it needs to be installed on site).', 'blackfyre' ) ?></p>
			</div>
			<div class="vc_automapper-toolbar"><a href="#" class="button button-primary"
			                                      id="vc_automapper-add-btn"><?php _e( 'Map Shortcode', 'blackfyre' ) ?></a>
			</div>
		...
		public function renderMapFormTpl() {
			...
			<script type="text/html" id="vc_automapper-add-form-tpl">
				<label for="vc_atm-shortcode-string"
				       class="vc_info"><?php _e( 'Shortcode string', 'blackfyre' ) ?></label>
				...
				<div class="vc_wrapper">
					...
					<div class="vc_buttons">
						<a href="#" id="vc_atm-parse-string"
						   class="button button-primary vc_parse-btn"><?php _e( 'Parse Shortcode', 'blackfyre' ) ?></a>
						<a href="#" class="button vc_atm-cancel"><?php _e( 'Cancel', 'blackfyre' ) ?></a>
					</div>
				</div>
				<span
					class="description"><?php _e( 'Enter valid shortcode (Example: [my_shortcode first_param="first_param_value"]My shortcode content[/my_shortcode]).', 'blackfyre' ) ?></span>
				</div>
			</script>
			...
			<script type="text/html" id="vc_automapper-form-tpl">
				...
				<div class="vc_wrapper">
					...
					<div class="vc_field vc_is-container">
						<label for="vc_atm-is-container"><input type="checkbox" name="is_container"
						                                        ...
						                                        value=""> <?php _e( 'Include content param into shortcode', 'blackfyre' ) ?>
						</label>
					</div>
				</div>
				<div class="vc_line"></div>
				<div class="vc_wrapper">
					...
					<a href="#" id="vc_atm-add-param"
					   class="button vc_add-param">+ <?php _e( 'Add Param', 'blackfyre' ) ?></a>

					<div class="vc_params" id="vc_atm-params-list"></div>
				</div>
				<div class="vc_buttons">
					<a href="#" id="vc_atm-save"
					   class="button button-primary"><?php _e( 'Save Changes', 'blackfyre' ) ?></a>
					<a href="#" class="button vc_atm-cancel"><?php _e( 'Cancel', 'blackfyre' ) ?></a>
					<a href="#" class="button vc_atm-delete"><?php _e( 'Delete', 'blackfyre' ) ?></a>
				</div>
			</script>
			<script type="text/html" id="vc_atm-form-param-tpl">
				<div class="vc_controls vc_controls-row controls vc_clearfix"><a
						...
						title="<?php _e( 'Drag row to reorder', 'blackfyre' ) ?>" data-vc-control="move"><i
							class="vc_icon"></i></a><span class="vc_row_edit_clone_delete"><a
							class="vc_control column_delete vc_delete-param" href="#"
							title="<?php _e( 'Delete this param', 'blackfyre' ) ?>"><i class="vc_icon"></i></a></span>
				</div>
							...
							<div class="wpb_element_wrapper">
								<div class="vc_fields vc_clearfix">
									<div class="vc_param_name vc_param-field">
										<label><?php _e( 'Param name', 'blackfyre' ) ?></label>
										<# if(param_name === 'content'){#>
											<span class="vc_content"><?php _e( 'Content', 'blackfyre' ) ?></span>
											<input type="text" style="display: none;" name="param_name"
											       value="{{ param_name }}"
											       placeholder="<?php _e( 'Required value', 'blackfyre' ) ?>"
											       class="vc_param-name"
											       data-system="true">
							<span class="description"
							      style="display: none;"><?php _e( 'Use only letters, numbers and underscore.', 'blackfyre' ) ?></span>
											<# } else { #>
												<input type="text" name="param_name" value="{{ param_name }}"
												       placeholder="<?php _e( 'Required value', 'blackfyre' ) ?>"
												       class="vc_param-name">
								<span
									class="description"><?php _e( 'Please use only letters, numbers and underscore.', 'blackfyre' ) ?></span>
												<# } #>
									</div>
									<div class="vc_heading vc_param-field">
										<label><?php _e( 'Heading', 'blackfyre' ) ?></label>
										<input type="text" name="heading" value="{{ heading }}"
										       placeholder="<?php _e( 'Input heading', 'blackfyre' ) ?>"
										<# if(type == 'hidden'){ #> disabled="disabled"
											<# } #> >
						<span
							class="description"><?php _e( 'Heading for field in shortcode edit form.', 'blackfyre' ) ?></span>
									</div>
									<div class="vc_type vc_param-field">
										<label><?php _e( 'Field type', 'blackfyre' ) ?></label>
										<select name="type">
											<option value=""><?php _e( 'Select field type', 'blackfyre' ) ?></option>
											<option
												value="textfield"<?php echo '<# if(type==="textfield") { #> selected="selected"<# } #>' ?>><?php _e( 'Textfield', 'blackfyre' ) ?></option>
											<option
												value="dropdown"<?php echo '<# if(type==="dropdown") { #> selected="selected"<# } #>' ?>><?php _e( 'Dropdown', 'blackfyre' ) ?></option>
											<option
												value="textarea"<?php echo '<# if(type==="textarea") { #> selected="selected"<# } #>' ?>><?php _e( 'Textarea', 'blackfyre' ) ?></option>
											<# if(param_name === 'content'){#>
												<option
													value="textarea_html"<?php echo '<# if(type==="textarea_html") { #> selected="selected"<# } #>' ?>><?php _e( 'Textarea HTML', 'blackfyre' ) ?></option>
												<# } #>
													<option
														value="hidden"<?php echo '<# if(type==="hidden") { #> selected="selected"<# } #>' ?>><?php _e( 'Hidden', 'blackfyre' ) ?></option>

										</select>
							<span
								class="description"><?php _e( 'Field type for shortcode edit form.', 'blackfyre' ) ?></span>
									</div>
									<div class="vc_value vc_param-field">
										<label><?php _e( 'Default value', 'blackfyre' ) ?></label>
										<input type="text" name="value" value="{{ value }}" class="vc_param-value">
						<span
							class="description"><?php _e( 'Default value or list of values for dropdown type (Note: separate by comma).', 'blackfyre' ) ?></span>
									</div>
									<div class="description vc_param-field">
										<label><?php _e( 'Description', 'blackfyre' ) ?></label>
										<textarea name="description" placeholder=""
										<# if(type == 'hidden'){ #> disabled="disabled"
											<# } #> >{{ description }}</textarea>
												<span
													class="description"><?php _e( 'Enter description for parameter.', 'blackfyre' ) ?></span>
									</div>
								</div>
							</div>
			</script>
		...
		public function renderTemplates() {
			...
			<script type="text/html" id="vc_automapper-item-tpl">
				...
				<span class="vc_automapper-item-controls">
                    <a href="#" class="vc_automapper-delete-btn" title="<?php _e( 'Delete', 'blackfyre' ) ?>"></a>
                    <a href="#" class="vc_automapper-edit-btn" title="<?php _e( 'Edit', 'blackfyre' ) ?>"></a>
                </span>
			</script>
			<?php
