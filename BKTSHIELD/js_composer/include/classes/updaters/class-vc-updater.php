...
class Vc_Updater {
	...
	public function upgradeFilterFromEnvato( $reply, $package, $updater ) {
		...
		if ( ( isset( $updater->skin->plugin ) && $updater->skin->plugin === vc_plugin_name() ) ||
			...
			$updater->strings['download_envato'] = __( 'Downloading package from envato market...', 'blackfyre' );
			$updater->skin->feedback( 'download_envato' );
			$package_filename = 'js_composer.zip';
			$res = $updater->fs_connect( array( WP_CONTENT_DIR ) );
			if ( ! $res ) {
				return new WP_Error( 'no_credentials', __( "Error! Can't connect to filesystem", 'blackfyre' ) );
			}
			...
			if ( ! vc_license()->isActivated() || empty( $username ) || empty( $api_key ) || empty( $purchase_code ) ) {
				return new WP_Error( 'no_credentials', __( 'To receive automatic updates license activation is required. Please visit <a href="' . admin_url( 'admin.php?page=vc-updater' ) . '' . '" target="_blank">Settings</a> to activate your Visual Composer.', 'blackfyre' ) );
			}
			...
			if ( ! isset( $result['download-purchase']['download_url'] ) ) {
				return new WP_Error( 'no_credentials', __( 'Error! Envato API error' . ( isset( $result['error'] ) ? ': ' . $result['error'] : '.' ), 'blackfyre' ) );
			}
			...
			return new WP_Error( 'no_credentials', __( 'Error on unzipping package', 'blackfyre' ) );
		}
