...
Class Vc_Templates_Panel_Editor implements Vc_Render {
		...
		if ( 'my_templates' == $category['category'] ) {
			$category['output'] = '
				<div class="vc_column vc_col-sm-12">
					<div class="vc_element_label">' . esc_html( 'Save current layout as a template', 'blackfyre' ) . '</div>
					<div class="vc_input-group">
						<input name="padding" class="vc_form-control wpb-textinput vc_panel-templates-name" type="text" value=""
						       placeholder="' . esc_attr( 'Template name', 'blackfyre' ) . '">
						<span class="vc_input-group-btn"> <button class="vc_btn vc_btn-primary vc_btn-sm vc_template-save-btn">' . esc_html( 'Save Template', 'blackfyre' ) . '</button></span>
					</div>
					<span class="vc_description">' . esc_html( 'Save layout and reuse it on different sections of this site.', 'blackfyre' ) . '</span>
				</div>';
			...
			if ( ! empty( $category['templates'] ) ) {
				foreach ( $category['templates'] as $template ) {
					$name = isset( $template['name'] ) ? esc_html( $template['name'] ) : esc_html( __( 'No title', 'blackfyre' ) );
	...
	public function renderTemplateWindowMyTemplates( $template_name, $template_data ) {
		...
		echo '</a>' .
		     ...
		     ' title="';
		esc_attr_e( 'Delete template', 'blackfyre' );
		echo '"' .
		     'data-template_id="';
		echo esc_attr( $template_data['unique_id'] );
		echo '"><i' .
	...
	public function getAllTemplates() {
		...
		if ( apply_filters( 'vc_show_user_templates', true ) ) {
			...
			$arr_category = array(
				...
				'category_name' => __( 'My Templates', 'blackfyre' ),
				'category_description' => __( 'Append previously saved template to the current layout.', 'blackfyre' ),
				'category_weight' => 10,
			);
		...
		if ( ! empty( $default_templates ) ) {
			$arr_category = array(
				...
				'category_name' => __( 'Default Templates', 'blackfyre' ),
				'category_description' => __( 'Append default template to the current layout.', 'blackfyre' ),
				'category_weight' => 11,
			);
			$category_templates = array();
