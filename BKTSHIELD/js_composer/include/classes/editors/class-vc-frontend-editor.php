...
Class Vc_Frontend_Editor implements Vc_Editor_Interface {
	...
	function renderEditor() {
		...
		if ( $this->post && $this->post->post_status === 'auto-draft' ) {
			$post_data = array(
				...
				'post_status' => 'draft',
				'post_title' => ''
			); // , 'post_title' => __('No title', 'blackfyre')
	...
	function getLeftButtons() {
		return array(
			array( 'add_element', '<li class="vc_show-mobile"><a href="#" class="vc_element_button" data-model-id="vc_element" id="vc_add-new-element" title="' . __( 'Add new element', 'blackfyre' ) . '"></a></li>' ),
			// array('add_row', '<li><a href="#" class="vc_row_button" data-tag="vc_row" id="vc_add-new-row">'.__('Add row', 'blackfyre').'</a></li>'),
			array( 'templates', '<li><a href="#" class="vc_templates_button"  id="vc_templates-editor-button" title="' . __( 'Templates', 'blackfyre' ) . '"></a></li>' )
		);
	}
	*/
	/**
	...
	function setEditorTitle() {
		return sprintf( __( 'Edit %s with Visual Composer', 'blackfyre' ), $this->post_type->labels->singular_name );
	}
	...
	function setEmptyTitlePlaceholder( $title ) {
		return ! is_string( $title ) || strlen( $title ) == 0 ? __( '(no title)', 'blackfyre' ) : $title;
	}
	...
	function renderEditButton( $link ) {
		if ( $this->showButton() ) {
			return $link . ' <a href="' . self::getInlineUrl() . '" id="vc_load-inline-editor" class="vc_inline-link">' . __( 'Edit with Visual Composer', 'blackfyre' ) . '</a>';
		}
	...
	function renderRowAction( $actions ) {
		...
		if ( $this->showButton( $post->ID ) ) {
			$actions['edit_vc'] = '<a
		href="' . $this->getInlineUrl( '', $post->ID ) . '">' . __( 'Edit with Visual Composer', 'blackfyre' ) . '</a>';
		}
