	...
	public function render() {
		...
		if ( ! empty( $deprecated ) ) {
			$output .= '<div class="vc_row vc_shortcode-edit-form-deprecated-message"><div class="vc_col-sm-12 wpb_element_wrapper">' .
			           vc_message_warning( sprintf( __( 'You are using outdated element, it is deprecated since version %s.', 'blackfyre' ), $this->setting( 'deprecated' ) ) ) .
			           '</div></div>';
		}
		$output .= '<div class="' . implode( ' ',
				$editor_css_classes ) . '" data-title="' . htmlspecialchars( __( 'Edit',
					'blackfyre' ) . ' ' . __( $this->setting( 'name' ), "js_composer" ) ) . '">';
		if ( is_array( $params ) ) {
