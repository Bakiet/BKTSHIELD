	...
	abstract class WPBakeryShortCode extends WPBakeryVisualComposerAbstract {
		...
		protected function registerCss( $param ) {
			if ( is_array( $param ) && ! empty( $param ) ) {
				...
				wp_register_style( $name, $param, array( 'blackfyre' ), WPB_VC_VERSION );
			}
		}
		...
		protected function loadTemplate( $atts, $content = null ) {
			...
			if ( $this->html_template ) {
				...
				trigger_error( sprintf( __( 'Template file is missing for `%s` shortcode. Make sure you have `%s` file in your theme folder.', 'blackfyre' ), $this->shortcode, 'wp-content/themes/your_theme/vc_templates/' . $this->shortcode . '.php' ) );
			}
...
---------------------------------------------------------- */
		...
		public function getColumnControls( $controls, $extended_css = '' ) {
			...
			$controls_add = ''; //' <a class="column_add" href="#" title="'.sprintf(__('Add to %s', 'blackfyre'), strtolower($this->settings('name'))).'"></a>';
			$controls_edit = ' <a class="vc_control column_edit" href="#" title="' . sprintf( __( 'Edit %s', 'blackfyre' ), strtolower( $this->settings( 'name' ) ) ) . '"><span class="vc_icon"></span></a>';
			$controls_delete = ' <a class="vc_control column_clone" href="#" title="' . sprintf( __( 'Clone %s', 'blackfyre' ), strtolower( $this->settings( 'name' ) ) ) . '"><span class="vc_icon"></span></a> <a class="column_delete" href="#" title="' . sprintf( __( 'Delete %s', 'blackfyre' ), strtolower( $this->settings( 'name' ) ) ) . '"><span class="vc_icon"></span></a>';
		...
		public function singleParamHtmlHolder( $param, $value ) {
			...
			if ( ! empty( $param['admin_label'] ) && $param['admin_label'] === true ) {
				$output .= '<span class="vc_admin_label admin_label_' . $param['param_name'] . ( empty( $value ) ? ' hidden-label' : '' ) . '"><label>' . __( $param['heading'], 'blackfyre' ) . '</label>: ' . $value . '</span>';
			}
	...
	abstract class WPBakeryShortCodesContainer extends WPBakeryShortCode {
		...
		public function getColumnControls( $controls = full, $extended_css = '' ) {
			...
			if ( $extended_css == 'bottom-controls' ) {
				$control_title = sprintf( __( 'Append to this %s', 'blackfyre' ), strtolower( $this->settings( 'name' ) ) );
			} else {
				$control_title = sprintf( __( 'Prepend to this %s', 'blackfyre' ), strtolower( $this->settings( 'name' ) ) );
			}

			$controls_move = '<a class="vc_control column_move" data-vc-control="move" href="#" title="' . sprintf( __( 'Move this %s', 'blackfyre' ), strtolower( $this->settings( 'name' ) ) ) . '"><span class="vc_icon"></span></a>';
			$controls_add = '<a class="vc_control column_add" data-vc-control="add" href="#" title="' . $control_title . '"><span class="vc_icon"></span></a>';
			$controls_edit = '<a class="vc_control column_edit" data-vc-control="edit" href="#" title="' . sprintf( __( 'Edit this %s', 'blackfyre' ), strtolower( $this->settings( 'name' ) ) ) . '"><span class="vc_icon"></span></a>';
			$controls_clone = '<a class="vc_control column_clone" data-vc-control="clone" href="#" title="' . sprintf( __( 'Clone this %s', 'blackfyre' ), strtolower( $this->settings( 'name' ) ) ) . '"><span class="vc_icon"></span></a>';
			$controls_delete = '<a class="vc_control column_delete" data-vc-control="delete" href="#" title="' . sprintf( __( 'Delete this %s', 'blackfyre' ), strtolower( $this->settings( 'name' ) ) ) . '"><span class="vc_icon"></span></a>';
