...
Class Vc_Column_Offset {
	...
	public function __construct( $settings, $value ) {
		...
		$this->column_width_list = array(
			__( '1 column - 1/12', 'blackfyre' ) => '1',
			__( '2 columns - 1/6', 'blackfyre' ) => '2',
			__( '3 columns - 1/4', 'blackfyre' ) => '3',
			__( '4 columns - 1/3', 'blackfyre' ) => '4',
			__( '5 columns - 5/12', 'blackfyre' ) => '5',
			__( '6 columns - 1/2', 'blackfyre' ) => '6',
			__( '7 columns - 7/12', 'blackfyre' ) => '7',
			__( '8 columns - 2/3', 'blackfyre' ) => '8',
			__( '9 columns - 3/4', 'blackfyre' ) => '9',
			__( '10 columns - 5/6', 'blackfyre' ) => '10',
			__( '11 columns - 11/12', 'blackfyre' ) => '11',
			__( '12 columns - 1/1', 'blackfyre' ) => '12'
		);
	}
	...
	public function sizeControl( $size ) {
		if ( $size === 'sm' ) {
			return '<span class="vc_description">' . __( 'Default value from width attribute', 'blackfyre' ) . '</span>';
		}
		$empty_label = $size === 'xs' ? '' : __( 'Inherit from smaller', 'blackfyre' );
	...
	public function offsetControl( $size ) {
		...
		$empty_label = $size === 'xs' ? __( 'No offset', 'blackfyre' ) : __( 'Inherit from smaller', 'blackfyre' );
		$output = '<select name="vc_' . $size . '_offset_size" class="vc_column_offset_field" data-type="offset-' . $size . '">'
		          . '<option value="" style="color: #ccc;">' . $empty_label . '</option>'
		          . ( $size === 'xs' ? '' :
				'<option value="' . $prefix . '0" style="color: #ccc;">' . __( 'No offset', 'blackfyre' ) . '</option>'
			);
		foreach ( $this->column_width_list as $label => $index ) {
			$value = $prefix . $index;
			$output .= '<option value="' . $value . '"' . ( in_array( $value, $this->data ) ? ' selected="true"' : '' ) . '>' . $label . '</option>';
