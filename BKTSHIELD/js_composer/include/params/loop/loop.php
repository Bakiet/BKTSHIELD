...
function vc_loop_form_field( $settings, $value ) {
	...
	return '<div class="vc_loop">'
	       ...
	       . '<a href="#" class="button vc_loop-build ' . $settings['param_name'] . '_button" data-settings="' . rawurlencode( json_encode( $settings['settings'] ) ) . '">' . __( 'Build query', 'blackfyre' ) . '</a>'
...
class VcLoopSettings {
	...
	function __construct( $value, $settings = array() ) {
		$this->parts = array(
			'size' => __( 'Post Count', 'blackfyre' ),
			'order_by' => __( 'Order By', 'blackfyre' ),
			'order' => __( 'Order', 'blackfyre' ),
			'post_type' => __( 'Post types', 'blackfyre' ),
			'authors' => __( 'Author', 'blackfyre' ),
			'categories' => __( 'Categories', 'blackfyre' ),
			'tags' => __( 'Tags', 'blackfyre' ),
			'tax_query' => __( 'Taxonomies', 'blackfyre' ),
			'by_id' => __( 'Individual Posts/Pages', 'blackfyre' )
		);
	...
	public function parse_order_by( $value ) {
		return $this->parseDropDown( $value, array(
			//array('none', __("None", 'blackfyre')),
			array( 'date', __( "Date", 'blackfyre' ) ),
			'ID',
			array( 'author', __( "Author", 'blackfyre' ) ),
			array( 'title', __( "Title", 'blackfyre' ) ),
			//'name',
			array( 'modified', __( "Modified", 'blackfyre' ) ),
			//'parent',
			array( 'rand', __( "Random", 'blackfyre' ) ),
			array( 'comment_count', __( "Comment count", 'blackfyre' ) ),
			array( 'menu_order', __( "Menu order", 'blackfyre' ) ),
			//'meta_value',
			//'meta_value_num',
			//'post__in'
		) );
	...
	public function parse_order( $value ) {
		return $this->parseDropDown( $value, array(
			array( 'ASC', __( "Ascending", 'blackfyre' ) ),
			array( 'DESC', __( "Descending", 'blackfyre' ) )
		) );
	}
