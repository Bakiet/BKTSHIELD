...
function vc_custom_css_admin_notice() {
	...
	$message_important = __( 'Important notice', 'blackfyre' );
	if ( is_object( $current_screen ) && isset( $current_screen->id ) && 'visual-composer_page_vc-color' == $current_screen->id ) {
		$message = __( 'You have an outdated version of Visual Composer Design Options. It is required to review and save it.', 'blackfyre' );
		...
		$html = '<p><strong>' . esc_html( $message_important ) . '</strong>: ' . esc_html( $message ) . '</p>'
		        . '<p><a ' . implode( ' ', $btnAtts ) . '>'
		        . __( 'Open Design Options', 'blackfyre' ) . '</a></p>';
	}
