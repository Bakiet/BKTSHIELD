...
<form action="options.php" method="post" id="vc_settings-<?php echo $tab ?>"
	...
	<?php if ( 'general' === $tab && vc_pointers_is_dismissed() ): ?>
		<table class="form-table">
			<tr>
				<th scope="row"><?php _e( 'Guide tours', 'blackfyre' ) ?></th>
				<td>
					<a href="#" class="button vc_pointers-reset-button"
					   id="vc_settings-vc-pointers-reset"
					   data-vc-done-txt="<?php _e( 'Done', 'blackfyre' ) ?>"><?php _e( 'Reset', 'blackfyre' ) ?></a>

					<p class="description indicator-hint"><?php _e( 'Guide tours are shown in VC editors to help you to start working with editors. Yo can see them again by clicking button above.', 'blackfyre' ) ?></p>
				</td>
			</tr>
		</table>
	<?php endif; ?>
	...
	?>
	<?php if ( $tab !== 'updater' ): ?>
		<?php submit_button( __( 'Save Changes', 'blackfyre' ), 'primary', 'submit_btn', true, $submit_button_attributes ); ?>
	<?php endif; ?>
