...
while ( $my_query->have_posts() ) {
	...
	if ( $grid_link != 'link_no' ) {
		$link = '<a class="more-link" href="' . get_permalink( $post_id ) . '"' . $link_target . ' title="' . sprintf( esc_attr__( 'Permalink to %s', 'blackfyre' ), the_title_attribute( 'echo=0' ) ) . '">' . __( "Read more", 'blackfyre' ) . '</a>';
	}
	...
	if ( $grid_link != 'link_no' ) {
		...
		if ( $grid_link == 'link_post' ) {
			$link_image_start = '<a class="link_image" href="' . get_permalink( $post_id ) . '"' . $link_target . ' title="' . sprintf( esc_attr__( 'Permalink to %s', 'blackfyre' ), the_title_attribute( 'echo=0' ) ) . '">';
			$link_title_start = '<a class="link_title" href="' . get_permalink( $post_id ) . '"' . $link_target . ' title="' . sprintf( esc_attr__( 'Permalink to %s', 'blackfyre' ), the_title_attribute( 'echo=0' ) ) . '">';
...
$categories_list = wp_list_categories(array(
	...
	'show_option_none'   => __('No categories', 'blackfyre'),
	'echo' => false
));
*/
	...
	$categories_list_output .= '<li class="active"><a href="#" data-filter="*">' . __( 'All', 'blackfyre' ) . '</a></li>';
...
if ( $teasers ) {
	...
	$teasers = __( "Nothing found.", 'blackfyre' );
}
