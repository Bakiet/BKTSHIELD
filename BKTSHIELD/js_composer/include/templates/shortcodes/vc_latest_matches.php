...
<?php
        ...
        if ( $clan ) {
        	$team_id = blackyfyre_return_team_id_by_post_id( $post->ID );
...
<ul class="clanwar-list">
       <li>
        ...
        $obj = new stdClass();
        $obj->id = 0;
        $obj->title = __('All', 'blackfyre');
        $obj->abbr = __('All', 'blackfyre');
        $obj->icon = 0;
		if(!empty($games1))
        ...
        ?>
       <li class="<?php echo esc_attr($class); ?>"><a href="#<?php echo esc_attr($link); ?>" title="<?php echo esc_attr($game->title); ?>"><?php echo esc_html($game->abbr); ?></a><div class="clear"></div></li>
       <?php  } endfor; ?>
        </ul>
        <div class="clear"></div>
    </li>
   ...
   foreach($matches as $i => $match) {
    if($match->status == 'active' || $match->status == 'done'){
       if($clan){
            ...
            $team1id = $match->team1;
            $logo1 = $wpdb->get_results($wpdb->prepare('SELECT logo FROM %s ` WHERE `id`= %s ',$table['teams'],$team1id  ));
            $gameabr = $wpdb->get_results($wpdb->prepare('SELECT abbr FROM %s WHERE `id`= %s ',$table['games'],$gameid  ));
            $thumb1 = $logo1[0]->logo;
            $img_url1 = wp_get_attachment_url( $thumb1,'full'); //get img URL
            $image1 = blackfyre_aq_resize( $img_url1, 25, 25, true ); //resize & crop img

            $team2id = $match->team2;
            $logo2 = $wpdb->get_results($wpdb->prepare('SELECT logo FROM %s WHERE `id`= %s ', $table['teams'], $team2id   ));
            $gameabr = $wpdb->get_results($wpdb->prepare('SELECT abbr FROM %s WHERE `id`= %s ', $table['games'], $gameid)  );
            $thumb2 = $logo2[0]->logo;
	...
	?>
			...
			<div class="wrap">
				...
				<?php else : ?>
				<div class="scores <?php echo $wld_class; ?>"><?php echo sprintf(__('%d:%d', 'blackfyre'), $t1, $t2); ?></div>
				<?php endif; ?>
                <?php if(!$image1){ $image1 = get_template_directory_uri().'/img/defaults/clan25x25.jpg';  } ?>
                <?php if(!$image2){ $image2 = get_template_directory_uri().'/img/defaults/clan25x25.jpg';  } ?>
				<div class="match-wrap">

						<img src="<?php echo $image1;?>" class="clan1img" alt="<?php echo esc_attr($match->title); ?>" />

						<span class="vs"><?php _e("vs","blackfyre");?></span>
						<div class="opponent-team">
						<img src="<?php echo $image2;?>" class="clan1img" alt="<?php echo esc_attr($match->title); ?>" />
						</div>
