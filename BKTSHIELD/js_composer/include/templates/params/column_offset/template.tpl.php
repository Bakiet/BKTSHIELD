<div class="vc_column-offset" data-column-offset="true">
	<?php if ( vc_settings()->get( 'not_responsive_css' ) === '1' ): ?>
		<div class="wpb_alert wpb_content_element vc_alert_rounded wpb_alert-warning">
			<div class="messagebox_text">
				<p><?php printf( __( 'Responsive design settings are currently disabled. You can enable them in Visual Composer <a href="%s">settings page</a> by unchecking "Disable responsive content elements".', 'blackfyre' ), admin_url( 'admin.php?page=vc-general' ) ) ?></p>
			</div>
		</div>
	<?php endif; ?>
	...
	<table class="vc_table vc_column-offset-table">
		<tr>
			<th>
				<?php _e( 'Device', 'blackfyre' ) ?>
			</th>
			<th>
				<?php _e( 'Offset', 'blackfyre' ) ?>
			</th>
			<th>
				<?php _e( 'Width', 'blackfyre' ) ?>
			</th>
			<th>
				<?php _e( 'Hide on device?', 'blackfyre' ) ?>
			</th>
		</tr>
		<?php foreach ( $sizes as $key => $size ): ?>
			<tr class="vc_size-<?php echo $key ?>">
				<td class="vc_screen-size vc_screen-size-<?php echo $key ?>">
					<span class="vc_icon" title="<?php _e( $size, 'blackfyre' ) ?>"></span>
				</td>
				<td>
...
<script type="text/javascript">
	...
	<?php echo json_encode(array(
			'inherit' => __('Inherit: ', 'blackfyre'),
			'inherit_default' => __('Inherit from default', 'blackfyre')
		)) ?>
</script>
